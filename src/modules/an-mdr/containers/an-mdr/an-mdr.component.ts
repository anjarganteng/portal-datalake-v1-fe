...
import { AnMdrInterface } from './../../services/an-mdr.service';

@Component({
    selector: 'sb-an-mdr',
    templateUrl: './an-mdr.component.html',
    styleUrls: ['an-mdr.component.scss'],
})
export class AnMdrComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnMdrInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            kodeMerchant: ['', Validators.maxLength(30)],
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'mdr/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Mdr.xlsx');
            })
    }
    ...
    showAdd() {
        ...
        //     this.form.controls.kodeMerchant.enable();

        //     if (!this.form.controls.kodeMerchant) {
        //         this.form.addControl('kodeMerchant', new FormControl(''));
        //     }
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        this.form?.controls.kodeMerchant.disable();
        this.Api.get(
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                // if (this.form.controls.kodeMerchant.value == '') {
                //     this.form.removeControl('kodeMerchant');
                // }

                if (this.form.controls.kodeMerchant.disabled) {
                    this.form.controls.kodeMerchant.enable();
                }
				
                this.Api.post(
                    this.Api.URLServiceAntasena + 'mdr/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'mdr/find-all',
            ...
            columns: [{ data: 'kodeMerchant' }, { data: 'rate' }],
        };
