...
import { AnKasvalasInterface } from './../../services/an-kasvalas.service';

@Component({
    selector: 'sb-an-kasvalas',
    templateUrl: './an-kasvalas.component.html',
    styleUrls: ['an-kasvalas.component.scss'],
})
export class AnKasvalasComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnKasvalasInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            flagDebet: ['', Validators.required],
            flagKredit: ['', Validators.required],
            new: [],
        });
        this.dEdit = {
            kodeTransaksi: '',
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'kas-valas/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Kas Valas.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            this.form.controls.kodeTransaksi.enable();
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        this.form?.controls.kodeTransaksi.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'kas-valas/find-by-id?id=' + 
                data.kodeTransaksi
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.kodeTransaksi.disabled) {
                    this.form.controls.kodeTransaksi.enable();
                }
                
                this.Api.post(
                    this.Api.URLServiceAntasena + 'kas-valas/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'kas-valas/find-all',
