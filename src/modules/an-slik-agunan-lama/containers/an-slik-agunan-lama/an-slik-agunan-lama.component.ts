...
import { AnSlikAgunanLamaInterface } from './../../services/an-slik-agunan-lama.service';

@Component({
    selector: 'sb-an-slik-agunan-lama',
    templateUrl: './an-slik-agunan-lama.component.html',
    styleUrls: ['an-slik-agunan-lama.component.scss'],
})
export class AnSlikAgunanLamaComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnSlikAgunanLamaInterface> | undefined;
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'slik-agunan-lama/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Mapping Fasilitas Agunan.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            this.form.controls.noSlikLama.enable();
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        this.form?.controls.noSlikLama.disable();
        this.form?.controls.noAgunanLama.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'slik-agunan-lama/find-by-id?id=' + 
                data.noSlikLama + '&id2=' + data.noAgunanLama
        ).subscribe(
            data => {
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'slik-agunan-lama/delete-by-id?id=' +
                                data.noSlikLama + '&id2=' + data.noAgunanLama,
                            {
                                noSlikLama: data.jenisInstrumen,
                            }
                        ).subscribe(
                            dataResult => {
                                this.loading = false;
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.noSlikLama.disabled) {
                    this.form.controls.noSlikLama.enable();
                }
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'slik-agunan-lama/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'slik-agunan-lama/find-all',
            ...
            columns: [{ data: 'embedSlikAgunanLama.noSlikLama' }, { data: 'embedSlikAgunanLama.noAgunanLama' }],
        };
