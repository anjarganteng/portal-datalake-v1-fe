...
import { AnInfraApmkInterface } from './../../services/an-infra-apmk.service';

@Component({
    selector: 'sb-an-infra-apmk',
    templateUrl: './an-infra-apmk.component.html',
    styleUrls: ['an-infra-apmk.component.scss'],
})
export class AnInfraApmkComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnInfraApmkInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            deviceId: [],
            kodeLokasi: ['', Validators.required],
            sektorUsaha: ['', Validators.required]
        });
        this.dEdit = {
            deviceId: '',
            kodeLokasi: '',
    ...
    showAdd() {
        if (this.form) {
            ...
            if (!this.form.controls.deviceId) {
                this.form.addControl('deviceId', new FormControl(''));
            }
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.deviceId.value == '') {
                    this.form.removeControl('deviceId');
                }
				
                this.Api.post(
                    this.Api.URLServiceAntasena + 'infra-apmk/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'infra-apmk/find-all',
            ...
            columns: [{ data: 'deviceId' }, { data: 'kodeLokasi' }],
        };
