...
import { AnApplicationBranchInterface } from './../../services/an-application-branch.service';

@Component({
    selector: 'sb-an-application-branch',
    templateUrl: './an-application-branch.component.html',
    styleUrls: ['an-application-branch.component.scss'],
})
export class AnApplicationBranchComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnApplicationBranchInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            uuidBranch: [],
            nameBranch: ['', Validators.required],
	    new: [],
        });
        this.dEdit = {
            uuidBranch: '',
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        // this.form?.controls.uuidBranch.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
            'application-branch/find-by-id?uuid_branch=' +
            data.uuidBranch
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'application-branch/delete-by-id?uuid_branch=' +
                                data.uuidBranch,
                            {
                                uuidBranch: data.uuidBranch,
                            }
                        ).subscribe(
                            dataResult => {
                                this.loading = false;
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.uuidBranch.disabled) {
                    this.form.controls.uuidBranch.enable();
                }
				
                this.Api.post(
                    this.Api.URLServiceAntasena + 'application-branch/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'application-branch/find-all',
