...
import { AnAtbInterface } from './../../services/an-atb.service';

@Component({
    selector: 'sb-an-atb',
    templateUrl: './an-atb.component.html',
    styleUrls: ['an-atb.component.scss'],
})
export class AnAtbComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnAtbInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            jenisAset: [],
            tglPerolehan: ['', Validators.required],
            tglMulai: ['', Validators.required],
            jthTempo: ['', Validators.required],
            jenisValuta: ['', Validators.required],
            metodeUkur: ['', Validators.required],
            jmlBlnLalu: ['', Validators.required],
            jmlDebet: ['', Validators.required],
            jmlKredit: ['', Validators.required],
            jmlLainnya: ['', Validators.required],
            jmlBlnLaporan: ['', Validators.required],
            akumulasiAmortasi: ['', Validators.required],
            new: [],
        });
        this.dEdit = {
            jenisAset: '',
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'atb/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Atb.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            this.form.controls.jenisAset.enable();
            this.form.controls.tglPerolehan.enable();
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        this.form?.controls.jenisAset.disable();
        this.form?.controls.tglPerolehan.disable();
        this.form?.controls.tglMulai.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'atb/find-by-id?id=' + 
                data.jenisAset + '&id2=' + data.tglPerolehan + '&id3=' + data.tglMulai
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.jenisAset.disabled) {
                    this.form.controls.jenisAset.enable();
                }

                if (this.form.controls.tglPerolehan.disabled) {
                    this.form.controls.tglPerolehan.enable();
                }
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'atb/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'atb/find-all',
            ...
            columns: [{ data: 'embedAtb.tglMulai' }],
        };
