...
import { AnGolpemilikInterface } from './../../services/an-golpemilik.service';

@Component({
    selector: 'sb-an-golpemilik',
    templateUrl: './an-golpemilik.component.html',
    styleUrls: ['an-golpemilik.component.scss'],
})
export class AnGolpemilikComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnGolpemilikInterface> | undefined;
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'gol-pemilik-individu/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Gol Pemilik Individu.xlsx');
            })
    }
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'gol-pemilik-individu/delete-by-id?id=' +
                                data.tipeGolPemilik,
                            {
                                tipeGolPemilik: data.tipeGolPemilik,
                            }
                        ).subscribe(
                            dataResult => {
                                this.loading = false;
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.tipeGolPemilik.value == '') {
                    this.form.removeControl('tipeGolPemilik');
                }
				
                this.Api.post(
                    this.Api.URLServiceAntasena + 'gol-pemilik-individu/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'gol-pemilik-individu/find-all',
