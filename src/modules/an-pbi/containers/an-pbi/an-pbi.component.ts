...
import { AnPbiInterface } from './../../services/an-pbi.service';

@Component({
    selector: 'sb-an-pbi',
    templateUrl: './an-pbi.component.html',
    styleUrls: ['an-pbi.component.scss'],
})
export class AnPbiComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnPbiInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            dalamLuarNegeri: ['', Validators.required],
            ...
            sandiBank: ['', Validators.required],
            noRekening: ['', Validators.required],
            mulai: ['', Validators.required],
            jatuhTempo: ['', Validators.required],
            klasifikasiAset: ['', Validators.required],
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'pbi/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Pbi.xlsx');
            })
    }
    ...
    regenerateData() {
        if (confirm('Are you sure you want to do this?')) {
            this.Api.post(
            this.Api.URLServiceAntasena + 'pbi/regenerate-data?namaProses=' +
            'FORM_PENEMPATAN_BI',
            {
                namaProses: 'FORM_PENEMPATAN_BI'
            }
            ).subscribe(
                data => {
                    if (data.status) {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'pbi/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'pbi/find-all',
