...
import { AnAtiInterface } from './../../services/an-ati.service';

@Component({
    selector: 'sb-an-ati',
    templateUrl: './an-ati.component.html',
    styleUrls: ['an-ati.component.scss'],
})
export class AnAtiComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnAtiInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            sumberPerolehan: ['', Validators.required],
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'ati/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Ati.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            this.form.controls.nomorAset.enable();
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        this.form?.controls.nomorAset.disable();
        this.Api.get(
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.nomorAset.disabled) {
                    this.form.controls.nomorAset.enable();
                }
                
                this.Api.post(
                    this.Api.URLServiceAntasena + 'ati/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'ati/find-all',
