...
import { AnAsetlainnyaInterface } from './../../services/an-asetlainnya.service';

@Component({
    selector: 'sb-an-asetlainnya',
    templateUrl: './an-asetlainnya.component.html',
    styleUrls: ['an-asetlainnya.component.scss'],
})
export class AnAsetlainnyaComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnAsetlainnyaInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            kodeCoa: [],
            keteranganJenisLainnya: ['', Validators.required],
            jenisAset: ['', Validators.required],
            golonganDebitur: ['', Validators.required],
            hubunganDebitur: ['', Validators.required],
            pendBknPend: ['', Validators.required],
            new: [],
        });
        this.dEdit = {
            kodeCoa: '',
            keteranganJenisLainnya: '',
            jenisAset: '',
            golonganDebitur: '',
            hubunganDebitur: '',
            pendBknPend: '',
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'aset-lainnya/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Aset Lainnya.xlsx');
            })
    }
    ...
    regenerateData() {
        if (confirm('Are you sure you want to do this?')) {
            this.Api.post(
                this.Api.URLServiceAntasena + 'aset-lainnya/regenerate-data?namaProses=' +
                'FORM_ASET_LAINNYA',
                {
                    namaProses: 'FORM_ASET_LAINNYA'
                }
                ).subscribe(
                    data => {
                        if (data.status) {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'aset-lainnya/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'aset-lainnya/find-all',
            ...
            columns: [{ data: 'kodeCoa' }, { data: 'jenisAset' }],
        };
