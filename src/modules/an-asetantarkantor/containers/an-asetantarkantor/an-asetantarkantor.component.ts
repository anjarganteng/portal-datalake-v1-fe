...
import { AnAsetantarkantorInterface } from './../../services/an-asetantarkantor.service';

@Component({
    selector: 'sb-an-asetantarkantor',
    templateUrl: './an-asetantarkantor.component.html',
    styleUrls: ['an-asetantarkantor.component.scss'],
})
export class AnAsetantarkantorComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnAsetantarkantorInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            kodeCoa: ['', [Validators.required, Validators.maxLength(10)]],
            deskripsiCoa: ['', [Validators.required, Validators.maxLength(100)]],
            usahaKantor: ['', [Validators.required, Validators.maxLength(3)]],
            statusKantor: [],
            negaraKantor: ['', [Validators.required, Validators.maxLength(5)]],
            jenisAset: ['', [Validators.required, Validators.maxLength(10)]],
            sukuBungaRp: [],
            sukuBungaVl: [],
            new: [],
        });
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'aset-antar-kantor/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Aset Antar Kantor.xlsx');
            })
    }
    ...
    regenerateData() {
        if (confirm('Are you sure you want to do this?')) {
            this.Api.post(
                this.Api.URLServiceAntasena + 'aset-antar-kantor/regenerate-data?namaProses=' +
    ...
    showEdit(data: any) {
        ...
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'aset-antar-kantor/find-by-id?id=' + 
                data.kodeCoa
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'aset-antar-kantor/delete-by-id?id=' +
                                data.kodeCoa,
                            {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'aset-antar-kantor/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'aset-antar-kantor/find-all',
            ...
            columns: [{ data: 'kodeCoa' }, { data: 'deskripsiCoa' }],
        };
