...
import { AnKodetransaksiInterface } from './../../services/an-kodetransaksi.service';

@Component({
    selector: 'sb-an-kodetransaksi',
    templateUrl: './an-kodetransaksi.component.html',
    styleUrls: ['an-kodetransaksi.component.scss'],
})
export class AnKodetransaksiComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnKodetransaksiInterface> | undefined;
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'kode-transaksi-lainnya/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Kode Transaksi Lainnya.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            ...
            if (!this.form.controls.kodeTransaksi) {
                this.form.addControl('kodeTransaksi', new FormControl(''));
            }
    ...
    //             'kode-transaksi-lainnya/find-by-id?id=' + 
    //             data.kodeTransaksi
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'kode-transaksi-lainnya/delete-by-id?id=' +
                                data.kodeTransaksi,
                            {
    ...
    onSubmit(f: NgForm) {
        console.log(f.value.kodeTransaksi);
    }
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.kodeTransaksi.value == '') {
                    this.form.removeControl('kodeTransaksi');
                }
				
                this.Api.post(
                    this.Api.URLServiceAntasena + 'kode-transaksi-lainnya/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'kode-transaksi-lainnya/find-all',
