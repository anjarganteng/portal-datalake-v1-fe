...
import { AnRatesbkInterface } from './../../services/an-ratesbk.service';

@Component({
    selector: 'sb-an-ratesbk',
    templateUrl: './an-ratesbk.component.html',
    styleUrls: ['an-ratesbk.component.scss'],
})
export class AnRatesbkComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnRatesbkInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            rateFlat: ['', Validators.required],
            rateEfektif: ['', Validators.required],
            new: [],
        });
        this.dEdit = {
            jenisPenggunaan: '',
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'rate-sbk/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Sbk.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            this.form.controls.jenisPenggunaan.enable();
            this.form.controls.jenisValuta.enable();
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        this.form?.controls.jenisPenggunaan.disable();
        this.form?.controls.jenisValuta.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'rate-sbk/find-by-id?id=' + 
                data.jenisPenggunaan + '&id2=' + data.jenisValuta
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.jenisPenggunaan.disabled) {
                    this.form.controls.jenisPenggunaan.enable();
                }

                if (this.form.controls.jenisValuta.disabled) {
                    this.form.controls.jenisValuta.enable();
                }
                
                this.Api.post(
                    this.Api.URLServiceAntasena + 'rate-sbk/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'rate-sbk/find-all',
            ...
            columns: [{ data: 'embedRateSbk.jenisPenggunaan' }],
        };
