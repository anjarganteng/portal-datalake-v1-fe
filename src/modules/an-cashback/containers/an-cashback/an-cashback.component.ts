...
import { AnCashbackInterface } from './../../services/an-cashback.service';

@Component({
    selector: 'sb-an-cashback',
    templateUrl: './an-cashback.component.html',
    styleUrls: ['an-cashback.component.scss'],
})
export class AnCashbackComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnCashbackInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            jnsSimpanan: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(1), Validators.pattern("^[a-zA-Z]*$")]],
            persentaseCashback: ['', Validators.required],
            new: [''],
        });
        this.dEdit = {
            noRek: '',
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'cashback/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Cashback.xlsx');
            })
    }
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.noRek.disabled) {
                    this.form.controls.noRek.enable();
                }
                
                this.Api.post(
                    this.Api.URLServiceAntasena + 'cashback/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'cashback/find-all',
            ...
            columns: [{ data: 'noRek' }, { data: 'jnsSimpanan' }, { data: 'persentaseCashback' }],
        };
