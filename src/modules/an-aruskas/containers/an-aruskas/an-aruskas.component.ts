...
import { AnAruskasInterface } from './../../services/an-aruskas.service';

@Component({
    selector: 'sb-an-aruskas',
    templateUrl: './an-aruskas.component.html',
    styleUrls: ['an-aruskas.component.scss'],
})
export class AnAruskasComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnAruskasInterface> | undefined;
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'arus-kas/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Proyeksi Arus Kas.xlsx');
            })
    }

    getSandiReferensi() {
        this.Api.get(
          this.Api.URLServiceAntasena + "parameter-arus-kas/get-all-by-flagrpp?flagRpp=true"
        ).subscribe(
          (dataResult) => {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'arus-kas/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'arus-kas/find-all',
