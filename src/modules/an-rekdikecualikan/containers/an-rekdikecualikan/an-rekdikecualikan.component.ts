...
import { AnRekdikecualikanInterface } from './../../services/an-rekdikecualikan.service';

@Component({
    selector: 'sb-an-rekdikecualikan',
    templateUrl: './an-rekdikecualikan.component.html',
    styleUrls: ['an-rekdikecualikan.component.scss'],
})
export class AnRekdikecualikanComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnRekdikecualikanInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            modul: [],
            rekeningDikecualikan: ['', Validators.required],
        });
        this.dEdit = {
            modul: '',
            rekeningDikecualikan: '',
        };
    ...
    showAdd() {
        if (this.form) {
            ...
            if (!this.form.controls.modul) {
                this.form.addControl('modul', new FormControl(''));
            }
    ...
    showEdit(data: any) {
        ...
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'rekening-dikecualikan/find-by-id?id=' + 
                data.modul
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'rekening-dikecualikan/delete-by-id?id=' +
                                data.modul,
                            {
                                modul: data.modul,
                            }
    ...
    onSubmit(f: NgForm) {
        console.log(f.value.rekeningDikecualikan);
    }
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.modul.value == '') {
                    this.form.removeControl('modul');
                }
				
                this.Api.post(
                    this.Api.URLServiceAntasena + 'rekening-dikecualikan/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'rekening-dikecualikan/find-all',
            ...
            columns: [{ data: 'modul' }, { data: 'rekeningDikecualikan' }],
        };
