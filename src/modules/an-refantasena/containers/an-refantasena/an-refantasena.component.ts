...
import { AnRefantasenaInterface } from './../../services/an-refantasena.service';

@Component({
    selector: 'sb-an-refantasena',
    templateUrl: './an-refantasena.component.html',
    styleUrls: ['an-refantasena.component.scss'],
})
export class AnRefantasenaComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnRefantasenaInterface> | undefined;
    closeResult: string | undefined;
    title = 'Referensi Antasena';
    loading = false;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            no: [],
            sandiExisting: ['', Validators.required],
        });
        this.dEdit = {
            no: '',
            sandiExisting: '',
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'referensi-antasena/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Referensi Antasena.xlsx');
            })
    }
    ...
    //         if (!this.form.controls.no) {
    //             this.form.addControl('no', new FormControl(''));
    //         }
    ...
    showEdit(data: any) {
        ...
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'referensi-antasena/find-by-id?id=' + 
                data.no
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'referensi-antasena/delete-by-id?id=' +
                                data.no,
                            {
                                no: data.no,
                            }
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                // if (this.form.controls.no.value == '') {
                //     this.form.removeControl('no');
                // }
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'referensi-antasena/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'referensi-antasena/find-all',
            ...
            columns: [{ data: 'no' }, { data: 'tabelReferensi' }],
        };
