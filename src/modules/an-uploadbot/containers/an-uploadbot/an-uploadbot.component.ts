...
import { AnUploadbotInterface } from './../../services/an-uploadbot.service';

@Component({
    selector: 'sb-an-uploadbot',
    templateUrl: './an-uploadbot.component.html',
    styleUrls: ['an-uploadbot.component.scss'],
})
export class AnUploadbotComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnUploadbotInterface> | undefined;
    ...
    getData(data: any, isEdit = false) {
        this.Api.get(this.Api.URLServiceAntasena + 'list-menu/get-all?form=' + 'bot').subscribe(
            dataResult => {
                this.dataData = [];
                let lastData;
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'upload-bot/save-excel', 
                    formData
                ).subscribe(
                    data => {
                        if (data.status) {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'upload-bot/find-all-bot',
            ...
            columns: [{ data: 'inputDate' }, { data: 'inputTime' }],
            order: [[0, 'desc'], [1, 'desc']],
        };
