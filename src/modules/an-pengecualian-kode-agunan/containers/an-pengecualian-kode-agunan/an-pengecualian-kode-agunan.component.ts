...
import { AnPengecualianKodeAgunanInterface } from './../../services/an-pengecualian-kode-agunan.service';

@Component({
    selector: 'sb-an-pengecualian-kode-agunan',
    templateUrl: './an-pengecualian-kode-agunan.component.html',
    styleUrls: ['an-pengecualian-kode-agunan.component.scss'],
})
export class AnPengecualianKodeAgunanComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnPengecualianKodeAgunanInterface> | undefined;
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'pengecualian-kode-agunan/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Pengecualian Kode Agunan.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            ...
            if (!this.form.controls.kodeAgunanCore) {
                this.form.addControl('kodeAgunanCore', new FormControl(''));
            }
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'pengecualian-kode-agunan/delete-by-id?id=' +
                                data.kodeAgunanCore,
                            {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.kodeAgunanCore.value == '') {
                    this.form.removeControl('kodeAgunanCore');
                }
				
                this.Api.post(
                    this.Api.URLServiceAntasena + 'pengecualian-kode-agunan/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'pengecualian-kode-agunan/find-all',
