...
import { AnCoaInterface } from './../../services/an-coa.service';

@Component({
    selector: 'sb-an-coa',
    templateUrl: './an-coa.component.html',
    styleUrls: ['an-coa.component.scss'],
})
export class AnCoaComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnCoaInterface> | undefined;
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'coa/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Coa.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            this.form.controls.coaInduk.enable();
            this.form.controls.coaDetail.enable();
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        this.form?.controls.coaInduk.disable();
        this.form?.controls.coaDetail.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'coa/find-by-id?id=' + 
                data.coaInduk + '&id2=' + data.coaDetail
        ).subscribe(
            data => {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.coaInduk.disabled) {
                    this.form.controls.coaInduk.enable();
                }

                if (this.form.controls.coaDetail.disabled) {
                    this.form.controls.coaDetail.enable();
                }
                
                this.Api.post(
                    this.Api.URLServiceAntasena + 'coa/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'coa/find-all',
            ...
            columns: [{ data: 'embedCoa.coaInduk' }, { data: 'embedCoa.coaDetail' }],
        };
