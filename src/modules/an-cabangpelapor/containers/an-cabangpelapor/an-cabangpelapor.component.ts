...
import { AnCabangpelaporInterface } from './../../services/an-cabangpelapor.service';

@Component({
    selector: 'sb-an-cabangpelapor',
    templateUrl: './an-cabangpelapor.component.html',
    styleUrls: ['an-cabangpelapor.component.scss'],
})
export class AnCabangpelaporComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnCabangpelaporInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            kodeCabangUtama: ['', Validators.required],
            kodeCoa: ['', Validators.required],
            namaCabang: ['', Validators.required],
            aliasCabang: ['', Validators.required],
            aliasWilayah: ['', Validators.required],
            kodeCabBi: ['', Validators.required],
            new: [],
        });
        this.dEdit = {
            statusData: '',
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'cabang-pelapor/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Cabang Pelapor.xlsx');
            })
    }
    ...
    showAdd() {
        ...
        //     if (!this.form.controls.kodeCabang) {
        //         this.form.addControl('kodeCabang', new FormControl(''));
        //     }
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'cabang-pelapor/delete-by-id?id=' +
                                data.kodeCabang,
                            {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.kodeCabang.disabled) {
                    this.form.controls.kodeCabang.enable();
                }
				
                this.Api.post(
                    this.Api.URLServiceAntasena + 'cabang-pelapor/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'cabang-pelapor/find-all',
