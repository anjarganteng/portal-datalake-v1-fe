...
import { AnPihakLawanInterface } from './../../services/an-pihak-lawan.service';

@Component({
    selector: 'sb-an-pihak-lawan',
    templateUrl: './an-pihak-lawan.component.html',
    styleUrls: ['an-pihak-lawan.component.scss'],
})
export class AnPihakLawanComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnPihakLawanInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            kdCabang: [],
            idPihakLawan: ['', Validators.required],
            jenisIdentitas: ['', Validators.required],
            nomorId: ['', Validators.required],
            jenisKelamin: ['', Validators.required],
            namaLengkap: ['', Validators.required],
            npwp: ['', Validators.required],
            kodeWn: ['', Validators.required],
            kodeNegara: ['', Validators.required],
            jenisKegUsaha: ['', Validators.required],
            hubBank: ['', Validators.required],
            golDeb: ['', Validators.required],
            kdLembaga: ['', Validators.required],
            kdRating: ['', Validators.required],
            tglRating: ['', Validators.required],
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'pihak-lawan/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Pihak Lawan.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            this.form.controls.kdCabang.enable();
            this.form.controls.idPihakLawan.enable();
    ...
    showEdit(data: any) {
        this.openModal();
        this.loading = true;
        this.form?.controls.kdCabang.disable();
        this.form?.controls.idPihakLawan.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'pihak-lawan/find-by-id?id=' + 
                data.kdCabang + '&id2=' + data.idPihakLawan
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.kdCabang.disabled) {
                    this.form.controls.kdCabang.enable();
                }

                if (this.form.controls.idPihakLawan.disabled) {
                    this.form.controls.idPihakLawan.enable();
                }
                
                this.Api.post(
                    this.Api.URLServiceAntasena + 'pihak-lawan/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'pihak-lawan/find-all',
            ...
            columns: [{ data: 'embedPihakLawan.kdCabang' }],
        };
