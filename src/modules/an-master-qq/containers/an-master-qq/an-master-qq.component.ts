...
import { AnMasterQqInterface } from './../../services/an-master-qq.service';

@Component({
    selector: 'sb-an-master-qq',
    templateUrl: './an-master-qq.component.html',
    styleUrls: ['an-master-qq.component.scss'],
})
export class AnMasterQqComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnMasterQqInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            cifQq: ['', [Validators.required, Validators.pattern("^[a-zA-Z0-9]*$")]],
            namaLengkap: ['', [Validators.required, Validators.pattern("^[a-zA-Z0-9 ]*$")]],
            new: [],
        });
        this.dEdit = {
            cifQq: '',
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'cifjoin-qq/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Cif Join QQ.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            this.form.controls.cifQq.enable();
	...
	console.log("tes");
        this.loading = true;
        this.form?.controls.cifQq.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'cifjoin-qq/find-by-id?id=' + 
                data.cifQq
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.cifQq.disabled) {
                    this.form.controls.cifQq.enable();
                }
                
                this.Api.post(
                    this.Api.URLServiceAntasena + 'cifjoin-qq/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'cifjoin-qq/find-all',
            ...
            columns: [{ data: 'cifQq' }, { data: 'namaLengkap' }],
        };
