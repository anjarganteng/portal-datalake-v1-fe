...
import { AnLiablainnyaInterface } from './../../services/an-liablainnya.service';

@Component({
    selector: 'sb-an-liablainnya',
    templateUrl: './an-liablainnya.component.html',
    styleUrls: ['an-liablainnya.component.scss'],
})
export class AnLiablainnyaComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnLiablainnyaInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            golonganKreditur: ['', Validators.required],
            hubunganKreditur: ['', Validators.required],
            negaraKreditur: ['', Validators.required],
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'liabilitas-lainnya/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Liabilitas Lainnya.xlsx');
            })
    }
    ...
    regenerateData() {
        if (confirm('Are you sure you want to do this?')) {
            this.Api.post(
                ...
                'FORM_LIABILITAS_LAINNYA',
                {
                    namaProses: 'FORM_LIABILITAS_LAINNYA'
                }
                ).subscribe(
                    data => {
                        if (data.status) {
    ...
    showEdit(data: any) {
        ...
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'liabilitas-lainnya/find-by-id?id=' + 
                data.kodeCoa
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'liabilitas-lainnya/delete-by-id?id=' +
                                data.kodeCoa,
                            {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'liabilitas-lainnya/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'liabilitas-lainnya/find-all',
