...
import { AnKursInterface } from './../../services/an-kurs.service';

@Component({
    selector: 'sb-an-kurs',
    templateUrl: './an-kurs.component.html',
    styleUrls: ['an-kurs.component.scss'],
})
export class AnKursComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnKursInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            kursJual: ['', Validators.required],
            kursBeli: ['', Validators.required],
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'kurs/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Kurs.xlsx');
            })
    }
    ...
    showAdd() {
        if (this.form) {
            ...
            this.form.controls.tanggal.enable();
    ...
    showEdit(data: any) {
        ...
        this.form?.controls.tanggal.disable();
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'kurs/find-by-id?id=' + data.mataUang 
                + '&id2=' + data.tanggal
                + '&id3=' + data.jenis
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena 
                            + 'kurs/delete-by-id?id=' + data.mataUang 
                            + '&id2=' + data.tanggal
                            + '&id3=' + data.jenis,
                            {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                if (this.form.controls.tanggal.disabled) {
                    this.form.controls.tanggal.enable();
                }
                
                this.Api.post(
                    this.Api.URLServiceAntasena + 'kurs/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'kurs/find-all',
            ...
            columns: [{ data: 'embedKurs.mataUang' }],
        };
