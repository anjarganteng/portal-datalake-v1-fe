...
import { AnGenerateGlBotInterface } from './../../services/an-generate-gl-bot.service';

@Component({
    selector: 'sb-an-generate-gl-bot',
    templateUrl: './an-generate-gl-bot.component.html',
    styleUrls: ['an-generate-gl-bot.component.scss'],
})
export class AnGenerateGlBotComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnGenerateGlBotInterface> | undefined;
    ...
    regenerateData() {
        if (confirm('Are you sure you want to do this?')) {
            this.Api.post(
                this.Api.URLServiceAntasena + 'manual-trigger/process-bot?namaProses=' +
                'bot_GL_NG',
                {
                    namaProses: 'bot_GL_NG'
                }
                ).subscribe(
                    data => {
                        if (data.status) {
                            this.modalService.dismissAll();
                            this.alertBoxSuccess(this.title, 'Success.');
                        } else {
        ...
        // this.Api.post(
        //     this.Api.URLServiceAntasena + 'manual-trigger/process?namaProses=' +
        //     'datawarehouse_GL',
        //     {
        //         namaProses: 'datawarehouse_GL'
        //     }
        //     ).subscribe(
        //         data => {
        //             if (data.status) {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'manual-trigger/find-all-bot',
