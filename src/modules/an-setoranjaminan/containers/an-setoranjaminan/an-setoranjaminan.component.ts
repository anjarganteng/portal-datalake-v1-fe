...
import { AnSetoranjaminanInterface } from './../../services/an-setoranjaminan.service';

@Component({
    selector: 'sb-an-setoranjaminan',
    templateUrl: './an-setoranjaminan.component.html',
    styleUrls: ['an-setoranjaminan.component.scss'],
})
export class AnSetoranjaminanComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnSetoranjaminanInterface> | undefined;
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
            ...
            tujuanSetoranJaminan: ['', Validators.required],
            golonganPemilik: ['', Validators.required],
            hubDgnPelapor: ['', Validators.required],
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'setoran-jaminan/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Setoran Jaminan.xlsx');
            })
    }
    ...
    regenerateData() {
        if (confirm('Are you sure you want to do this?')) {
            this.Api.post(
                ...
                'FORM_SETORAN_JAMIN',
                {
                    namaProses: 'FORM_SETORAN_JAMINAN'
                }
                ).subscribe(
                    data => {
                        if (data.status) {
            ...
            .result.then(
                result => {
                    ...
                    if (this.closeResult == 'dodelete') {
                        ...
                        this.Api.post(
                            this.Api.URLServiceAntasena +
                                'setoran-jaminan/delete-by-id?id=' +
                                data.kodeCoa,
                            {
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'setoran-jaminan/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'setoran-jaminan/find-all',
