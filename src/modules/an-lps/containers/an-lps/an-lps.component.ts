...
import { AnLpsInterface } from './../../services/an-lps.service';

@Component({
    selector: 'sb-an-lps',
    templateUrl: './an-lps.component.html',
    styleUrls: ['an-lps.component.scss'],
})
export class AnLpsComponent implements  AfterViewInit, OnDestroy, OnInit {
   form: FormGroup | undefined;
    ModelInterface: Observable<AnLpsInterface> | undefined;
    ...
    getExcelData() {
        this.http.get(this.Api.URLServiceAntasena + 'bunga-lps/download', { responseType: 'blob' })
            .subscribe((res: any) => {
                saveAs(res, 'Bunga Lps.xlsx');
            })
    }
    ...
    showEdit(data: any) {
        ...
        this.Api.get(
            this.Api.URLServiceAntasena + 
                'bunga-lps/find-by-id?id=' + 
                data.tglMulai + '&id2=' + data.jenisValuta
        ).subscribe(
            data => {
                const d = data.data;
                d.new = false;
    ...
    submit() {
        if (this.form) {
            if (this.form.valid) {
                ...
                this.Api.post(
                    this.Api.URLServiceAntasena + 'bunga-lps/save', 
                    this.form.value
                ).subscribe(
                    data => {
        ...
        that.dtOptions = {
            ...
            ajax: (dataTablesParameters: any, callback) => {
                this.Api.loadDataTable(
                    this.Api.URLServiceAntasena + 'bunga-lps/find-all',
            ...
            columns: [{ data: 'embedLps.tglMulai' }],
        };
